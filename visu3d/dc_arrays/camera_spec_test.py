...
def test_camera_spec_init(
  ...
  assert enp.compat.is_xnp_array(spec.px_from_cam @ x, xnp)

  x = _broadcast_to(xnp, [0, 0.0], (1,) * len(spec_shape) + (2,))
  assert enp.compat.is_xnp_array(spec.cam_from_px @ x, xnp)
...
def test_camera_spec_central_point(
  ...
  assert enp.compat.is_xnp_array(central_point_px, xnp)
